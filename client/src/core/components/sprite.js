...
export default class Sprite extends GameObject {
	...
	constructor(spriteConfig, position) {
		...
		this.#frameSize =
			frameSize instanceof Vector2 ? frameSize : new Vector2(16, 16);
	...
	set scale(scale) {
		if (!(Number.isFinite(scale) && scale >= 0)) {
			// eslint-disable-next-line max-len
			throw new TypeError(
				`Scale was expected to be a positive number, instead received: ${scale}`,
			);
		}
		this.#scale = scale;
	}
	#buildFrameMap() {
		...
		for (let v = 0; v < this.#vFrames; ++v) {
			for (let h = 0; h < this.#hFrames; ++h) {
				this.#frameMap.set(
					frameCount,
					new Vector2(h * this.#frameSize.x, v * this.#frameSize.x),
				);
				++frameCount;
			}
		}
	...
	step(deltaTime) {
		...
		this.#animationsManager.step(deltaTime * 0.5);
		this.#frame = this.#animationsManager.frame;
	}
	drawImage(ctx, startDrawX, startDrawY) {
